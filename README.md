...
日々の栄養バランスを考慮した食事案を提案する CLI ツール
`meal_recommender.py` を追加しました。カロリー目標とマクロ栄養素の割合を指定して、
朝食・昼食・夕食（必要ならスナック）を自動的に提案します。
...
| オプション             | 説明                                     |
|------------------------|------------------------------------------|
| `--calories <int>`     | 1 日あたりの目標カロリー (1200–3500 kcal)|
| `--protein <float>`    | たんぱく質のカロリー比率 (初期値 0.25)   |
| `--carb <float>`       | 炭水化物のカロリー比率 (初期値 0.5)     |
| `--fat <float>`        | 脂質のカロリー比率 (初期値 0.25)        |
| `--diet vegetarian`    | ベジタリアン向けメニューのみ表示         |
| `--no-snack` / `--snack` | スナックの提案を切り替え（既定は提案あり） |

比率は合計 1.0 になるよう指定してください。実行すると、メニューの詳細と 1 日分の総計
（カロリー／たんぱく質／炭水化物／脂質）が出力されます。

### Web UI 版

ブラウザから設定を調整しながらプランを確認したい場合は、標準ライブラリ製の簡易サーバーを起動します。

```bash
python meal_recommender_ui.py
```

`http://127.0.0.1:8000` を開くと、目標カロリーやマクロ比率、スナック有無を調整できるフォームと、
おすすめメニューが表示されます。入力に不備があるときは画面上部にエラーが表示され、
条件を修正しながらすぐに再計算できます。

> **コンテナ経由でブラウザからアクセスする場合**は、`--host 0.0.0.0` を指定して起動すると、
> 外部ブラウザからプレビューしやすくなります。
